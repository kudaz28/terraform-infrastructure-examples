...
if [ $# -ne 2 ]; then
    ...
    e.g. init.sh api-jva-email devh'
    exit 1
fi
...
sed -i "s/APPLICATION_NAME/${application_name}/g" ../../../base/security_groups/data.tf
sed -i "s/ENVIRONMENT/${environment}/g" backend.tf
sed -i "s/ENVIRONMENT/${environment}/g" ../../../base/security_groups/data.tf
